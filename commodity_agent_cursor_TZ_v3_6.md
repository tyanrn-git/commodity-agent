...
Версия: 3.6  
Статус: уточнено владельцем продукта перед Этапом 0, включая управление AI-бюджетом  
...
```text
Изучи PRODUCT_SPEC.md версии 3.6.
...
# 26. Зафиксированные решения перед разработкой

## 26.1. Источники истины и архитектурные решения

- Этот документ копируется в корень репозитория как `PRODUCT_SPEC.md` и является источником продуктовых требований.
- `ARCHITECTURE.md` ведёт разработчик; бизнес-значимые допущения утверждает владелец продукта.
- Решения, меняющие scope, модель сделки, внешнюю коммуникацию или расчёт экономики, требуют согласования владельца продукта.


## 26.1.1. AI Budget Settings

Добавить сущность `AIBudgetSettings`.
...
- user_id;
- monthly_budget_usd;
- first_warning_percent;
- second_warning_percent;
- hard_limit_enabled;
...
Изменения настроек не перезаписывают историю без следа: каждое изменение фиксируется в AuditLog.

## 26.2. Пользователи и аутентификация

- MVP рассчитан на одного трейдера.
- Используется один предсозданный admin-аккаунт.
- Публичная регистрация отсутствует.
- Сессии cookie-based: `HttpOnly`, `Secure`, `SameSite`.
- Все timestamps хранятся в UTC.
- Часовой пояс UI настраивается в профиле; значение по умолчанию — `Atlantic/Madeira`.

## 26.3. Инфраструктура

- Локальная разработка и первые тесты: Docker Compose.
- Пилот: один VPS или облачный сервер.
- Файлы на Этапах 0–2 хранятся в локальной файловой системе через интерфейс `ObjectStorage`.
- S3-compatible storage подключается перед облачным пилотом без изменения доменной логики.
- CI: GitHub Actions.
- Sentry подключается перед реальным email-пилотом.
- Daily backup обязателен при облачном/production-деплое.
- Локально должна существовать команда ручного backup.
- Секреты: `.env` локально, secrets платформы/CI в production.
- Код, API, схемы БД, логи и комментарии — на английском; UI — на русском.

## 26.4. Декомпозиция Этапа 1

### Этап 1a — без AI

- buyer-led Opportunity;
- загрузка документов;
- ручной Requirement;
- Source и Evidence;
...
- первым подключается OpenAI;
- конкретное имя модели задаётся конфигурацией, а не hardcode;
- structured outputs;
- usage logging;
- soft spending limit;
- возможность отключить AI;
- текстовые PDF, DOCX и XLSX;
- OCR сканов отложен;
- ручной импорт `.eml`;
- URL import только для публичных статических страниц.

## 26.5. Пилотный товар и golden set

- Первый товар: base oil SN500/SN150.
- Seed-каталог содержит только SN500 и SN150.
- ProductSpecificationProfile готовится разработчиком и утверждается трейдером.
- Golden set: реальные обезличенные документы плюс синтетические edge cases.
- Golden set готов до завершения Этапа 3.
- Для price, currency, quantity и Incoterm требуется 100% точность на утверждённом golden set.
- Для остальных размеченных полей целевой порог — не ниже 95%.

## 26.6. Контрагенты и RFQ

- Поиск контрагентов реализуется через provider abstraction.
- Ручное добавление всегда остаётся доступным.
- `DOMAIN_VERIFIED` требует валидного DNS/MX, совпадения домена с официальным сайтом и подтверждения пользователем.
- Язык RFQ выбирается отдельно; default — English.
- Используется библиотека утверждённых шаблонов.
- AI адаптирует шаблон, но не создаёт окончательное письмо без review.
- Подпись и реквизиты берутся из настроек компании и профиля пользователя.

## 26.7. Email loop

- Почтовый провайдер выбирается по фактическому рабочему mailbox.
- Gmail API используется для Gmail/Google Workspace; Microsoft Graph — для M365.
- В MVP импортируются только новые письма после подключения.
- Старые письма импортируются вручную через `.eml`.
- Неоднозначные письма попадают в `Unlinked Inbox`.
- Минимальный binding class задаётся кодовыми правилами.
- AI может предложить более высокий класс.
- Пользователь подтверждает класс перед отправкой.
- `NOT_REVIEWED` compliance показывает заметное предупреждение и требует отдельного подтверждения, но не создаёт абсолютную блокировку.

## 26.8. Расчёты

- FX: официальный ECB reference rate как базовый источник, snapshot на дату расчёта, ручная корректировка с комментарием.
- Таможня и пошлины: ручной ввод суммы или ставки с Evidence.
- Incoterms: версия 2020.
- Доменная модель поддерживает все Incoterms 2020.
- Первый UI выделяет EXW, FCA, FOB, CFR, CIF, CPT, CIP и DAP.
- Поддерживаются метрические единицы, bbl и US gallon.
- Все значения нормализуются в каноническую единицу продукта.
- `risk_adjusted_margin` не показывается без явно заданных пользователем вероятностей.
- В MVP реализуются только CURRENT и CONFIRMED.
- Tolerance хранится в ProductSpecificationProfile; ручное переопределение требует комментария.

## 26.9. Оферта

- Основной формат: email HTML/plain.
- Дополнительно допускается PDF из утверждённого шаблона.
- Реквизиты и брендинг предоставляет владелец продукта.
- До этого используется минимальный текстовый шаблон.
- Раскрытие поставщика управляется через `DealParty.disclosure_status`.
- По умолчанию название поставщика покупателю не раскрывается.

## 26.10. Мониторинг и дальнейшая автоматизация

- Первый источник окончательно выбирается после Этапа 5.
- Приоритетный кандидат: официальный источник MMTC/RCF/NFL по карбамиду.
- Приоритет способов доступа: API → RSS → email → static HTML → Playwright.
- ToS и robots.txt проверяются до реализации.
- Первый polling interval — один раз в сутки.
- Маржа не является обязательным фильтром до появления надёжных данных по цене и логистике.
- Supplier-led scenario следует после первого monitoring connector.
- Controlled automation откладывается до успешного ручного пилота; первым автоматизируется только follow-up.

## 26.11. Нерешённые входные данные владельца продукта

До соответствующих этапов необходимо определить:

- бюджет/лимит AI;
- фактический mailbox и домен;
- юридическое лицо и бренд;
- реальную buyer-led возможность для пилота;
- дополнительные KPI пилота.

# 27. Решения владельца продукта от 12 июля 2026 года

## 27.1. Бюджет AI

- Начальный месячный бюджет AI API для пилота: **100 USD**.
- Пользователь может изменять бюджет в личном кабинете без изменения кода и redeploy.
- В профиле/настройках должны быть доступны:
  - monthly_budget_usd;
  - first_warning_percent;
  - second_warning_percent;
  - hard_limit_enabled;
...
- Значения по умолчанию:
  ...
  - second warning: 90%;
  - hard limit: 100%;
  - reset day: первый день календарного месяца.
- При hard limit автоматические AI-вызовы останавливаются.
- Пользователь с правами Admin может:
  - увеличить или уменьшить лимит;
  - временно снять hard limit;
  - разрешить единичный override;
  - полностью отключить AI;
  - изменить модели по умолчанию.
- Любое изменение бюджета или лимитов:
  - записывается в AuditLog;
  - хранит старое и новое значение;
  - хранит пользователя и timestamp;
  - применяется только к новым AI-вызовам.
- UI должен показывать:
  - текущие расходы месяца;
  - остаток бюджета;
  - прогноз до конца месяца;
  - расходы по моделям;
  - расходы по операциям;
  - расходы по сделкам и ResearchCampaign.
- Бюджет относится к AI API и встроенным AI-tools; стоимость Cursor, hosting, search API, email и других внешних сервисов учитывается отдельно.
- Система обязана хранить usage и cost по:
  - модели;
  - операции;
  - сделке;
  - ResearchCampaign;
  - документу;
  - дню и месяцу.
- Модель выбирается по принципу минимально достаточной стоимости:
  - cost-efficient модель для extraction, классификации и простых черновиков;
  - более сильная модель только для сложного сопоставления, противоречий и финального commercial draft;
  - повторные вызовы ограничены;
  - кеширование и повторное использование результатов обязательны.
- Конкретные модели задаются конфигурацией и могут меняться без миграции данных.

## 27.2. Email и реквизиты компании

- Email-интеграция не настраивается на Этапах 0–2.
- До подключения mailbox система поддерживает:
  - экспорт готового письма;
  - копирование текста;
  - ручную отметку `SENT_EXTERNALLY`;
  - ручной импорт ответа через `.eml`, PDF или текст.
- Провайдер, mailbox, домен, юридическое лицо, бренд, подпись и реквизиты будут определены перед Этапом 3.
- Настройки компании должны быть отдельной конфигурационной сущностью и не хардкодиться.

## 27.3. Первый пилот начинается без готовой buyer-led возможности

На старте нет заранее известной реальной потребности покупателя.

Поэтому первый пилотный вертикальный сценарий изменяется:

```text
Заданные товар и география
→ поиск потенциальной потребности или предложения
→ создание Opportunity
→ поиск потенциальных покупателей и поставщиков
→ подготовка запросов
→ ручная отправка запросов до подключения mailbox
→ получение и импорт ответов
→ запросы перевозчикам и другим сервисным сторонам
→ сбор подтверждённых параметров
→ построение рабочей FulfilmentConfiguration
→ расчёт экономики
→ решение о продолжении или отклонении цепочки
```

Это называется `CHAIN_DISCOVERY_PILOT`.

Цель пилота — не доказать заранее известную сделку, а проверить, может ли система найти и подтвердить рабочую коммерческую цепочку:

- реальная или публично подтверждаемая потребность покупателя;
- минимум один потенциальный источник товара;
- минимум один реализуемый маршрут;
- необходимые сервисные стороны;
- достаточные данные для предварительного расчёта.

## 27.4. Новая сущность ResearchCampaign

Добавить сущность `ResearchCampaign`, которая описывает направленный поиск рабочей цепочки до появления конкретной Opportunity.
...
ResearchCampaign может породить:

- buyer-need Opportunity;
- supplier-offer Opportunity;
- несколько конкурирующих Opportunities;
- решение `NO_VIABLE_CHAIN_FOUND`.

ResearchCampaign не является Deal и не должна искусственно конвертироваться в Deal до появления достаточно конкретной коммерческой возможности.

## 27.5. Этап Chain Discovery

До первоначального buyer-led сценария добавить этап:

### Этап 1c — Chain Discovery Pilot

Пользовательский результат:

- система помогает найти потенциальную цепочку по выбранному товару;
- формирует список покупателей, поставщиков и сервисных сторон;
- создаёт адресные запросы;
- пользователь отправляет их вручную;
- ответы импортируются и превращаются в CommercialFact;
- система определяет, достаточно ли данных для создания Deal.

Критерии приёмки:

- создана одна ResearchCampaign;
- найдено минимум 3 потенциальных покупателя или публичных buyer needs;
- найдено минимум 3 потенциальных поставщика;
- найден минимум 1 возможный логистический маршрут;
- подготовлены отдельные запросы покупателю, поставщику и логистическому провайдеру;
- отправка может быть зафиксирована вручную;
- импортирован минимум один реальный ответ либо документально подтверждённая публичная котировка;
- создана минимум одна Opportunity;
- система явно показывает missing facts и причины, по которым цепочка пока не считается рабочей.

Не входит:

- автоматическая email-отправка;
- обещание покупателю цены;
- подтверждение сделки;
- обязательный положительный результат поиска.

## 27.6. KPI первого пилота

Для первого пилота не устанавливаются дополнительные коммерческие KPI.

Минимальный критерий успеха:

- система прошла полный исследовательский цикл без потери источников;
- подготовлены запросы всем основным типам сторон;
- получен или импортирован минимум один содержательный внешний ответ;
- создана минимум одна рассчитываемая конфигурация либо зафиксировано доказанное отсутствие рабочей конфигурации;
- каждое критичное значение имеет Evidence;
- пользователь может понять, почему цепочка является или не является перспективной.

Отсутствие прибыльной сделки не считается провалом пилота, если система корректно выявила причины и сохранила подтверждающие данные.

## 27.7. Будущее расширение после пилота

После успешного пилота продукт расширяется от pre-trade проработки к сопровождению фактической сделки.

Будущие функции:

### Deal execution documents

- checklist обязательных документов;
- шаблоны документов;
- версии и статусы;
- ответственные стороны;
- сроки получения;
- проверка комплектности;
- связь документа с партией, транспортным плечом и оплатой.
...
- фактически закупленное и проданное количество;
- фактические даты;
- фактический маршрут;
- фактические затраты;
- фактические платежи;
...
- демередж;
- претензии;
- отклонения от плановой конфигурации.

### Plan-versus-actual economics

Для закрытой сделки рассчитывать:

- planned revenue vs actual revenue;
- planned cost vs actual cost;
- planned margin vs actual margin;
- margin variance by cost category;
- financing days planned vs actual;
- FX variance;
- logistics variance;
- quantity variance;
- delay cost;
...
Эти функции не входят в первый MVP, но доменная модель должна сохранять возможность добавить:
